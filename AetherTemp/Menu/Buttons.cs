    ...
    internal class Buttons
    {
        public static ButtonInfo[][] buttons = new ButtonInfo[][]
        {
            ...
            new ButtonInfo[] { // Menu Settings
                ...
                new ButtonInfo { buttonText = $"Change Button Sound {(mods.num == 66f ? "" : mods.num == 8f ? "" : mods.num == 169f ? "" : "")}", method = () => { mods.GunSmoothNess(); foreach (var category in Buttons.buttons) foreach (var button in category) if (button.buttonText.StartsWith("Smoothness")) button.buttonText = $"Smoothness: {(mods.num == 66f ? "Keyboard" : mods.num == 8f ? "Normal" : mods.num == 169f ? "Creamy" : "Unknown")}"; }, isTogglable = false, toolTip = "Changes gun smoothness." }
